...
class Agent:
    ...
    def collect_policies(
        ...
        # Other nodes
            ...
            # If planning > 1 step, use the previous action to propagate state,
            # then evaluate preferences under the current action.
            transition_u = prev_action if (prev_action is not None and self.policy_length > 1) else node.u
            node.q_s_u = torch.einsum(
                'funk,fk->fun',
                self.B,  # (f, u, n, k): factor, u (action), next (state), kurrent (state)
                q_s  # (f, k)
            )[:, transition_u].squeeze()  # (f, u, n) -> (f, n)
        ...
        for child in node.children:
            ...
            subtree_EFEs, subtree_EFE_terms, sub_policy = self.collect_policies(
                child, node.q_s_u, node.u, new_policy_EFEs, policy_EFE_terms, new_policy)
